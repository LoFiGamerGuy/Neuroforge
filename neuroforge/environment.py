...
class WorkflowEnv:
    ...
    def _observe(self) -> Observation:
        ...
        failed = c in (HiddenCondition.TRANSIENT, HiddenCondition.PERMANENT)
        ...
        return Observation(float(self.t), float(self.t), float(self.t-self.last_progress), float(activity), float(output), float(success), self.recent_failures, self.retries, float(repetition), float(signal), self.cost, self.last_action)

    def step(self, action: Action) -> tuple[Observation, bool]:
        if self.done:
            raise RuntimeError("episode is complete")
        c = self._active_condition()
        needs = c in (HiddenCondition.TRANSIENT, HiddenCondition.PERMANENT, HiddenCondition.LOOPING, HiddenCondition.FALSE_PROGRESS)
        ...
        if not self.done:
            ...
            if c in (HiddenCondition.TRANSIENT, HiddenCondition.PERMANENT): self.recent_failures += 1
            self.t += 1
